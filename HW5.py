...
class CsvReader(FileReader):
    ...
    def get_name_rating(self):
        ...
        for i in range(len(self.data_dict['Name'])):
            tuples_list.append(
                (self.data_dict['Name'][i], self.data_dict['Rating'][i])
                )
    ...
    def most_common_month(self):
        ...
        for i in self.data_dict['Release Date']:
            month = i.split('/')[0]
            month_counts[month] = month_counts.get(month, 0)

        top_months = sorted(month_counts.items())
